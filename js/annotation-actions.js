...
window.SubDnDManager = {
    draggedItem: null,

    dragStart: function(event, topicoId, parentIndex, subIndex) {
        // Trava de segurança: O arrasto SÓ inicia se o usuário clicou no Handle
        if (!event.target.closest('.sub-drag-handle')) {
            event.preventDefault();
            return;
        }
        
        this.draggedItem = { topicoId, parentIndex, subIndex };
        event.currentTarget.classList.add('dragging'); 
        event.dataTransfer.effectAllowed = 'move';
        event.dataTransfer.setData('text/plain', ''); // Fix para Firefox
    },
    ...
    dragEnter: function(event) {
        event.preventDefault();
        const wrapper = event.currentTarget.closest('.sub-annotation-item');
        if (wrapper) wrapper.classList.add('drag-over');
    },
    ...
    dragLeave: function(event) {
        const wrapper = event.currentTarget.closest('.sub-annotation-item');
        if (!wrapper) return;
        // Evita pisca-pisca se o mouse passar por elementos filhos
        if (!wrapper.contains(event.relatedTarget)) {
            wrapper.classList.remove('drag-over');
        }
    },
    ...
    dragEnd: function(event) {
        // Limpeza de DOM Seguro
        // Se a operação foi cancelada e o nó ainda vive, limpa.
        if (document.contains(event.currentTarget)) {
            event.currentTarget.classList.remove('dragging');
        }
        // Limpeza global garante que classes residuais desapareçam.
        document.querySelectorAll('.sub-annotation-item.drag-over').forEach(el => el.classList.remove('drag-over'));
    },

    drop: function(event, targetTopicoId, targetParentIndex, targetSubIndex) {
        event.preventDefault();
        ...
        // 2. Validação arquitetural cross-group
        if (!src || src.topicoId !== targetTopicoId || src.parentIndex !== targetParentIndex) {
            exibirToast('Ideias secundárias só podem ser reordenadas dentro do próprio agrupamento.', 'aviso');
            return;
        }
        
        // 3. Otimização: se destino é igual origem, aborta (evita re-render)
        if (src.subIndex === targetSubIndex) return;

        // 4. Lógica de Mutação
        const topico = topicos.find(t => t.id === targetTopicoId);
        const subAnotacoes = topico.anotacoes[targetParentIndex].subAnotacoes;

        const [itemMovido] = subAnotacoes.splice(src.subIndex, 1);
        subAnotacoes.splice(targetSubIndex, 0, itemMovido);

        // 5. Persistência e Repaint
        renderizarTopicos(); // Recria o DOM
        salvarBackupAutomatico();
        exibirToast('Ordem das ideias atualizada!', 'sucesso');
    }
};
